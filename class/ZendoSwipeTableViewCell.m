...
-(void)setup{
    ...
    UIView *holderView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height-1)];
...
//    [self.contentView addSubview:holderView];//for debug
...
//    CGPoint position = CGPointMake(self.frame.size.width/2.0f, self.bounds.size.height/2.0f);
//    CGSize imgSize = [UIImage imageNamed:@"white_trash.png"].size;
//    CGRect rect = CGRectMake(position.x-imgSize.width-15, position.y-(imgSize.height/2), 25, 25);
...
- (void)onPanGestureRecognizer:(UIPanGestureRecognizer *)gesture{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan || gesture.state == UIGestureRecognizerStateChanged) {
        ...
        if (_needCheckState && _currentPercentage<0.55f && _currentPercentage>0.3f) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(mtzendoTableViewCell:enableStateChanged:)]) {
                [self.delegate mtzendoTableViewCell:self enableStateChanged:self.enabled];
            }
        }
    }
...
- (void)onDeleteBtn:(UIButton *)sender{
    ...
    SIAlertView *alertView = [[SIAlertView alloc] initWithTitle:nil andMessage:@"Are you sure to delete this alarm?"];
