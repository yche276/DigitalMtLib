...
- (void)setMode:(DMTProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == DMTProgressHUDModeDeterminate) {
...
- (void)updateProgress {
    [(DMTRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == DMTProgressHUDModeDeterminate) {
        self.indicator = [[DMTRoundProgressView alloc] initWithDefaultSize];
    }
    else {
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"DMTProgressHUDViewIsNillException" 
					format:@"The view used in the DMTProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.mode = DMTProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) { 
       ...
       // CGSize dims = [self.labelText sizeWithFont:self.labelFont];
		CGSize dims = [self.labelText sizeWithFont:self.labelFont constrainedToSize:CGSizeMake(500, 200) lineBreakMode:UILineBreakModeTailTruncation];
...
- (void)done {
    ...
    if(delegate != nil && [delegate conformsToProtocol:@protocol(DMTProgressHUDDelegate)]) {
...
@implementation DMTRoundProgressView
